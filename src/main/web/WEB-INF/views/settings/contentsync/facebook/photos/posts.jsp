...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
    <div class="main-grids">
        <div class="button"><a href="/settings/contentsync/facebook/photos/menu">Back</a></div>
        <h4 class="contentTitle">My Posts</h4>

        <div id="myPosts">
            <c:forEach items="${photos}" var="photo">
                <div>
                    <img class="photo" src="${photo.path}"/>
                    <form:form id="storeForm${photo.providerID}" cssClass="button photosForm"
                               action="/settings/contentsync/facebook/photos/posts/store"
                               method="post">
                        <input type="hidden" name="photoURL" value="${photo.path}">
                        <div class="buttonInput">
                            <input type="submit" value="Store Photo"/>
                        </div>
                    </form:form>
                </div>
            </c:forEach>
...
<script src="<spring:url value="/resources/js/contentSyncLoader.js"/>"></script>
<script>
    ...
    jQuery(document).ready(function ($) {
        $(function () {
            $('.photosForm').on('submit', function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: '/settings/contentsync/facebook/photos/posts/store',
                    data: $(this).serialize(),
                    success: function (data) {
                        var element = document.getElementById(self.id).parentNode;
                        element.parentNode.removeChild(element);
                        Materialize.toast('Photo successfully saved!',3000,'green rounded');
                    },
                    error: function (e) {
                        Materialize.toast('Photo failed to saved!',3000,'red rounded');
                    }
                });
                e.preventDefault();
            });
