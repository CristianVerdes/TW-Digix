...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
    <div class="main-grids">
        ...
        <h4 class="contentTitle">Google Documents</h4>

        <c:forEach items="${documents}" var="document">

            <div>
                <a href="${document.path}"><img class="photo" src="${document.thumbnailURL}" alt="Preview"/></a>
                <form:form id="documentForm${document.providerID}" cssClass="button documentForm" action="/settings/contentsync/google/documents/store" method="post">
                    <input type="hidden" name="providerID" value="${document.providerID}">
                    <div class="buttonInput">
                        <input type="submit" value="Store Document"/>
                    </div>
                </form:form>
            </div>
    ...
    jQuery(document).ready(function ($) {
        $(function () {
            $('.documentForm').on('submit', function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: '/settings/contentsync/google/documents/store',
                    data: $(this).serialize(),
                    success: function (data) {
                        var element = document.getElementById(self.id).parentNode;
                        element.parentNode.removeChild(element);
                        Materialize.toast('Document successfully saved!',3000,'green rounded');
                    },
                    error: function (e) {
                        Materialize.toast('Document failed to saved!',3000,'red rounded');
                    }
                });
                e.preventDefault();
            });
