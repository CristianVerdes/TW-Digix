    ...
    <div class="drop-navigation drop-navigation">
        <ul class="nav nav-sidebar">
            ...
            <ul class="cl-effect-1">
                ...
                <li><a href="${videosURL}"><span class="glyphicon glyphicon-film"
                                                 aria-hidden="true"></span>Videos</a></li>
                <li class="active"><a href="${documentsURL}"><span class="glyphicon glyphicon-paperclip"
                                                                   aria-hidden="true"></span>Documents</a>
                </li>
            </ul>
...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
    <div class="main-grids">
        <h4 class="contentTitle">My Documents</h4>

        <c:if test="${currentPage > 1}">
            <div class="button"><a href="/digitalbox/documents?page=${currentPage - 1}">Previous Page</a></div>
        </c:if>
        <c:if test="${currentPage < counter}">
            <div class="button"><a href="/digitalbox/documents?page=${1 + currentPage}">Next Page</a></div>
        </c:if>

        <c:forEach items="${documents}" var="document">

            <div>
                <a href="${document.path}"><img class="photo" src="${document.thumbnailURL}" alt="Preview"/></a>
                <form:form cssClass="button" action="/digitalbox/editdocument" method="post"
                           modelAttribute="documentForm">
                    <input type="hidden" name="contentID" value="${document.contentID}">>
                    <div class="buttonInput">
                        <input type="submit" value="Edit Document"/>
                    </div>
                </form:form>
                <form:form id="deleteForm${document.contentID}" cssClass="button deleteForm"
                ...
                <c:if test="${document.postedDate == null}">
                    <form:form cssClass="button" action="/digitalbox/postdocument" method="post"
                               modelAttribute="documentForm">
                        <input type="hidden" name="contentID" value="${document.contentID}">>
                        <div class="buttonInput">
                            <input type="submit" value="Post Document"/>
                        </div>
                    </form:form>
                </c:if>
            </div>
    ...
    jQuery(document).ready(function ($) {
        $(function () {
            $('.documentForm').on('submit', function (e) {
                ...
                $.ajax({
                    type: 'post',
                    url: '/digitalbox/postdocument',
                    data: $(this).serialize(),
                    success: function (data) {
                        var element = document.getElementById(self.id);
                        element.parentNode.removeChild(element);
                        Materialize.toast('Document successfully posted!',2000,'green rounded');
                    },
                    error: function (e) {
                        Materialize.toast('Document failed to post!',2000,'red rounded');
                    }
                });
                e.preventDefault();
            });
    ...
    jQuery(document).ready(function ($) {
        $(function () {
            $('.deleteForm').on('submit', function (e) {
                ...
                $.ajax({
                    type: 'get',
                    url: '/digitalbox/deletedocument',
                    data: $(this).serialize(),
                    success: function (data) {
                        var element = document.getElementById(self.id).parentNode;
                        element.parentNode.removeChild(element);
                        Materialize.toast('Document successfully deleted!',3000,'yellow rounded');
                    },
                    error: function (e) {
                        Materialize.toast('Document failed to delete!',3000,'red rounded');
                    }
                });
                e.preventDefault();
            });
